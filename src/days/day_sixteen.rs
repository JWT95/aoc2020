...
impl FromStr for Field {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
       ...
       Ok(Field {
           ...
           range_one: Range { start: captures[2].parse::<u32>()? , end: captures[3].parse::<u32>()? + 1 },
           range_two: Range { start: captures[4].parse::<u32>()? , end: captures[5].parse::<u32>()? + 1 }
       })
    }
}
...
fn part_one(fields: &[Field], nearby: &Vec<Vec<u32>>) {
    let invalid: u32= nearby.iter().map(|x| ticket_is_invalid(&x, fields)).flatten().sum();

    println!("{:?}", invalid);
}

fn part_two(fields: &[Field], nearby: &Vec<Vec<u32>>, yours: &[u32]) {
    let valid = nearby.iter().filter(|x| ticket_is_invalid(&x, fields).is_empty());
    ...
    for ticket in valid {
        for i in 0..num_fields {
            for j in 0..num_fields {
                if !fields[j].range_one.contains(&ticket[i]) && !fields[j].range_two.contains(&ticket[i]) {
                    possible_fields.get_mut(&(j as u32)).unwrap().remove(&(i as u32));
                }
            }
        }
    }
    ...
    // Now reduce
    let mut not_done = {
        possible_fields.values().find(|v| v.len() > 1).is_some()
    };

    while not_done {
        let singletons: Vec<(u32, u32)> = possible_fields.iter().filter(|(_k, v)| v.len() == 1).map(|(k, v)| (k.clone(), v.iter().nth(0).unwrap().clone())).collect();

        for singleton in singletons {
            for (_k, v) in  possible_fields.iter_mut().filter(|(k, _v)| singleton.0 != **k) {
    ...
    let answer = (0..6).map(|x| possible_fields[&x].iter().nth(0).unwrap()).map(|x| yours[*x as usize]).fold(1 as u64, |acc, x| acc*(x as u64));
...
pub fn day_sixteen() -> Result<()> {
    ...
    let blanks: Vec<_> = lines.iter().enumerate().filter(|(_, b)| **b == String::new()).map(|(a, _)| a).collect();
    ...
    let nearby = nearby[2..].iter().map(|x| x.split(',').map(|y| y.parse::<u32>().unwrap()).collect::<Vec<u32>>()).collect();
    let yours = yours[2].split(',').map(|y| y.parse::<u32>().unwrap()).collect::<Vec<u32>>();
