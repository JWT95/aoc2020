...
pub fn day_seventeen() -> Result<()> {
    let inputs: Vec<Vec<char>> = read_input("input/day_seventeen.txt")?.map(|x| x.chars().collect()).collect();
...
fn neighbors_active(x: i32, y: i32, z: i32, w: i32, input: &HashMap<(i32, i32, i32, i32), char>) -> u32 {
    let mut count = 0;
    for i in -1..2 {
        for j in -1..2 {
            for k in -1..2 {
                for ii in -1..2 {
                    if !(i==0 && j==0 && k==0 && ii==0) {
                        let val = input.get(&(x+i,y+j,z+k, w+ii)).unwrap_or_else(|| &INACTIVE);
                        if *val == ACTIVE {
                            count += 1;
                        }
                    }
